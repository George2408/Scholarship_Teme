...
export class NoteService {
  ...
  getNote(id: string): Observable<Note> {
    return this.httpClient.get<Note>(
      this.baseUrl + '/notes/' + `${id}` , this.httpOptions
    );
  }
  ...
  getFiltredSearch(searchWord: string): Observable<Note[]>
   {
    ...
    .pipe(
      map((notes: Note[]) => {
        return notes.filter(note => note.title.toLowerCase().includes(searchWord) || note.description.toLocaleLowerCase().includes(searchWord));
      })
    );
    //  this.searchWord.toLowerCase()
